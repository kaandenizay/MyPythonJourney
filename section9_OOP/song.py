...
def find_object(field, object_list):
    """
    Check object list to see if an object with a 'name' attribute equal to
    ...
    with open("albums.txt", "r") as albums:
        for line in albums:
            ...
            if new_artist is None:
                ...
                # otherwise create a new artist object and add it to the artist
...
def create_checkfile(artist_list):
    """Creates a checkfile from the object data for comparison with original file"""
    with open("checkfile.txt", "w") as checkfile:
        for artist in artist_list:
            for album in artist.albums:
                for song in album.tracks:
                    print("{0.name}\t{1.name}\t{1.year}\t{2.title}"
                          .format(artist, album, song), file=checkfile)

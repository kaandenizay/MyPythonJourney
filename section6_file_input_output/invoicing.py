...
def next_invoice_number(invoice_number: str) -> str:
    ...
    if invoice_year == year:
        ...
        invoice_year = year
        number = 1
    new_invoice_number = "{}-{:04d}".format(invoice_year, number)
...
def record_invoice(invoice_file: TextIO,
    ...
    if last_row:
        ...
        """
        Here, I'm not interested in invoice company or amount
        So, I'm showing them as "_", means they are not important for me
        after unpacking
        """
    ...
    print("{}\t{}\t{}".format(new_invoice_number,company,amount), file=invoice_file)
...
with open('../data/' + data_file, 'r+') as invoices:
